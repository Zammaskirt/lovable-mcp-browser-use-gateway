...
if not heavy:
    ...
    print("Heavy packages currently locked (these bloat the image on Fly):")
    for name, version in heavy:
        print(f"- {name}=={version}")
    raise SystemExit(
        "Heavy ML/CUDA packages are still pinned in uv.lock — the Saik0s engine currently "
        "depends on browser-use which pulls these in. Consider forking browser-use to strip "
        "ML extras or rebuilding uv.lock with a lighter upstream release."
    )
