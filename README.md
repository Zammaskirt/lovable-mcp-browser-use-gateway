...
what's being pulled from `uv.lock` (it exits non-zero when it finds heavy locks):
...
they remain in the image because they're locked. If you maintain a private deployment and need an even smaller image, consider
forking `browser-use` to strip the ML extras and regenerate `uv.lock` to shed the `torch`/`sentence-transformers` layer.
